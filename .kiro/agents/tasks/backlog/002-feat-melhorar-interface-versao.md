...
A rota `/api/versao` retorna apenas texto simples (ex: `Bia 4.3.0`). O objetivo desta tarefa é
criar/recriar uma **página visual dedicada** na rota `/versao` do frontend, que consuma essa API
e exiba as informações de versão com o mesmo padrão de design da aplicação BIA.

> Observação: uma tentativa anterior desta melhoria (task 001) foi implementada e posteriormente
> revertida. Esta tarefa reinicia o trabalho do zero, considerando o estado atual do código
> (não existe mais rota `/versao` nem componente de página dedicado no frontend).

## Objetivo

Melhorar a experiência visual da rota `/versao` substituindo a resposta de texto puro por uma
interface gráfica consistente com o restante da aplicação BIA.
...
Essa lógica **deve ser aproveitada** para construir a nova página, seja por reuso direto ou
